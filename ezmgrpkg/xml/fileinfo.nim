...
template queryKind(x: static string, isvar: static bool = false): string =
  var desc: LPWSTR
  var desc_n: UINT
  let path = (when isvar: varpart else: basepart) & x
  ...
  $desc
template queryOptionalKind(x: static string, isvar: static bool = false): string =
  var desc: LPWSTR
  var desc_n: UINT
  let path = (when isvar: varpart else: basepart) & x
  if VerQueryValue(pblock, path, cast[ptr PVOID](addr desc), addr desc_n) == 0:
...
proc parseModFile*(str: string): tuple[id: string, desc: ref ModDescription] =
  ...
  let varpart = r"\VarFileInfo\"
  result.id = queryKind("InternalName")
  result.desc.kind = parseEnum(queryKind("ModKind", true), modServerSide)
  ...
  result.desc.license = queryKind("Licence", true)
